...
class Collection:
    ...
    def retire_endpoints_and_sources(
        collection, collection_name, collection_df_to_retire
    ) -> None:
        """
        This method should be called from commands.py with the correct input.csv format, it should then be called per unique collection.

        Args:
            collection_dir (str): The name of the directory containing all the collections.
            collection_name (str): The name of the collection where endpoints and sources are to be retired from.
            collection_df_to_retire_df (str): A small dataframe of sources and endpoints to be retired within a colleciton, this should be
        ...
        try:
            ...
            endpoint_csv_df.loc[
                endpoint_csv_df["endpoint"].isin(
                    collection_df_to_retire_df["endpoint"]
                ),
                "end-date",
            ] = today_date
            ...
            source_csv_df.loc[
                source_csv_df["source"].isin(collection_df_to_retire_df["source"]),
                "end-date",
            ] = today_date
