...
def test_dirs(tmp_path_factory):
    ...
    additional_fields_dict = get_additional_field_csv_data(specification_dir)
...
def get_additional_field_csv_data(specification_dir: Path = None):
    return {
        "cardinality": [1, 1],
...
# def test_one(test_dirs):
#     print("")
#     print(f">>> test_two: {test_dirs}")
...
def test_pipeline_run(test_dirs):
    """
    This test uses commands.py pipeline_run to perform a full run-through
    of the full pipeline phases, using test data generated by the input
    ...
    issue_dir = test_dirs["collection_dir"]
    organisation_path = "tests/data/listed-building/organisation.csv"
    dataset_resource_dir = test_dirs["collection_dir"]
    test_endpoints = [dummy_endpoint, test_endpoint, dummy_endpoint[::-1]]
