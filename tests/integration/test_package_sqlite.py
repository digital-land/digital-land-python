...
def test_sqlite_package(mocker, tmpdir, setup_cwd_for_pipeline_ingest):
    ...
    expected_entity_result = [
        ("", "", 110042408, "2021-12-01", "", "WILK WOOD", "", "", "", "", "")
    ]
    expected_metadata_last_modified_result = "2021-12-14T14:57:07.714295"
    mocker.patch.object(
        SqlitePackage,
        "_get_current_datetime",
        return_value=expected_metadata_last_modified_result,
    )
    tables = {
        "entity": "dataset",
    }
    ...
    assert list(conn.execute("select * from metadata")) == [
        (1, expected_metadata_last_modified_result),
    ]
