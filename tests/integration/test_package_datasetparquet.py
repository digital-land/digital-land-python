...
def test_dataset_parquet_package(temp_dir):
    ...
    data = [
        [np.nan, 111, '2023-01-01', 2, 'badcfe', 'entry-date', 2, np.nan, 'zyx123', np.nan, '2023-01-01'],
        [np.nan, 111, '2023-01-01', 2, 'badcfe', 'entry-date', 2, np.nan, 'zyx123', np.nan, '2023-01-01'], # same
...
def test_load_fact_basic(test_dataset_parquet_package, temp_dir):
    ...
    assert len(df) == 11, "No. of facts is not correct"  # No of unique facts
...
def test_load_fact_resource_basic(test_dataset_parquet_package, temp_dir):
    ...
    assert len(df) == 20, "Not all data saved in fact-resource.parquet file"
...
def test_load_entities_basic(test_dataset_parquet_package, temp_dir):
    output_dir = temp_dir
    test_dataset_parquet_package.load_entities(test_dataset_parquet_package.input_paths, output_dir)
    ...
    assert len(df) == 10, "No. of entities is not correct"
...
# def test_load_pq_to_sqlite_basic(test_dataset_parquet_package):
