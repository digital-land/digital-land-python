...
def test_dataset_parquet_package(temp_dir):
    ...
    data = [
        ...
        [np.nan, 11, '2023-01-01', 2, 'abcdef', 'organisation', 2, np.nan, 'zyxwvu', np.nan, "local-authority:AAA"],
        ...
        [np.nan, 12, '2023-01-01', 2, 'abc123', 'organisation', 2, np.nan, 'zyxwvu', np.nan, "local-authority:BBB"],
        ...
        [np.nan, 13, '2023-01-01', 2, 'def456', 'organisation', 2, np.nan, 'zyxwvu', np.nan, "local-authority:CCC"],
        ...
        [np.nan, 14, '2023-01-01', 2, 'a1b2c3', 'organisation', 2, np.nan, 'zyxwvu', np.nan, "local-authority:DDD"]
    ]
    ...
    data = [
        [np.nan, 110, '2023-01-01', 2, 'badcfe', 'entry-date', 2, np.nan, 'zyx123', np.nan, '2023-01-01'],
        [np.nan, 110, '2023-01-01', 2, 'badcfe', 'entry-date', 2, np.nan, 'zyx123', np.nan, '2023-01-01'], # same
        [np.nan, 110, '2023-01-01', 2, 'badcfe', 'organisation', 2, np.nan, 'zyx123', np.nan, "local-authority:DDD"],
        [np.nan, 111, '2023-01-01', 2, 'fedcba', 'entry-date', 2, np.nan, 'zxy123', np.nan, '2023-01-01'],
        [np.nan, 111, '2023-02-01', 2, 'fedcba', 'entry-date', 2, np.nan, 'zxy123', np.nan, '2023-02-01'], #ent-date
        [np.nan, 111, '2023-02-01', 2, 'fedcba', 'organisation', 2, np.nan, 'zxy123', np.nan, "local-authority:EEE"],
        ...
        [np.nan, 112, '2023-01-01', 12, 'bcdefg', 'organisation', 2, np.nan, 'yxw456', np.nan, "local-authority:FFF"],
        ...
        [np.nan, 113, '2023-01-01', 2, 'cdefgh', 'organisation', 2, np.nan, 'xwv789', np.nan, "local-authority:GGG"],
        ...
        [np.nan, 114, '2023-01-01', 2, 'efghij', 'organisation', 1, np.nan, 'xyz123', np.nan, "local-authority:HHH"],
        ...
        [np.nan, 115, '2023-01-01', 2, 'defghi', 'organisation', 2, np.nan, 'uvw456', np.nan, "local-authority:III"],
        [np.nan, 116, '2023-01-01', 2, 'ihgfed', 'entry-date', 2, np.nan, 'rta357', np.nan, '2023-01-01'] # No org
    ]
...
def test_load_fact_basic(test_dataset_parquet_package, temp_dir):
    output_dir = temp_dir
    test_dataset_parquet_package.load_facts(test_dataset_parquet_package.input_paths, output_dir)
...
def test_load_fact_resource_basic(test_dataset_parquet_package, temp_dir):
    output_dir = temp_dir
    test_dataset_parquet_package.load_fact_resource(test_dataset_parquet_package.input_paths, output_dir)
...
def test_load_entities_basic(test_dataset_parquet_package, temp_dir):
    ...
    test_dataset_parquet_package.load_entities(test_dataset_parquet_package.input_paths, output_dir, temp_dir)
...
def test_load_pq_to_sqlite_basic(test_dataset_parquet_package, temp_dir):
    ...
    assert np.all(pd.isnull(df_sql['end-date'])), "Non-empty strings in end_date from fact_resource table"

    df_sql = pd.read_sql_query(f"SELECT * FROM fact", cnx)
    assert len(df_sql) > 0, "No data in fact table"
    assert np.all(pd.isnull(df_sql['end-date'])), "Non-empty strings in end_date from fact table"

    df_sql = pd.read_sql_query(f"SELECT * FROM entity", cnx)
    assert len(df_sql) > 0, "No data in entity table"
    assert np.any(pd.isnull(df_sql['geometry'])), "All geometries from entity table have values"
    cnx.close()
