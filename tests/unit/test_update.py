...
from digital_land.update import has_collected_resource, get_failing_endpoints, get_entries_between_keys
...
def test_get_entries_between_keys():
    ...
    start_idx, end_idx = get_entries_between_keys(1, 10, len(register), lambda idx: register[idx])
    assert (start_idx, end_idx) == (0,8)

    start_idx, end_idx = get_entries_between_keys(0, 8, len(register), lambda idx: register[idx])
    assert (start_idx, end_idx) == (0,5)

    start_idx, end_idx = get_entries_between_keys(3, 7, len(register), lambda idx: register[idx])
...
def test_get_failing_endpoints_filter_inactive():
    ...
    failing_endpoints = get_failing_endpoints(LOG_ENTRIES, ENDPOINT_ENTRIES, first_date=date(2017, 1, 1))
    expected_result = {"AAA", "CCC", "EEE"}
    assert(set(failing_endpoints.keys()) == expected_result and len(failing_endpoints["CCC"]["failure_dates"]) == 2)

def test_get_failing_endpoints_filter_dates():
    # Test date filtering
    failing_endpoints = get_failing_endpoints(LOG_ENTRIES, ENDPOINT_ENTRIES, first_date=date(2020, 10, 23))
    expected_result = {"EEE"}
