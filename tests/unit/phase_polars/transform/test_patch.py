...
def test_patch_regex():
    ...
    lf = pl.LazyFrame({"grade": ["II", "II*", "2", "2*", "2 Star", "1", "3"]})
...
def test_patch_url_with_special_chars():
    ...
    assert result["OrganisationURI"].to_list() == [
        "patch_organisation",
        "https://other.com",
    ]
...
def test_patch_global_pattern():
    patches = {"": {"^test$": "replaced"}}

    lf = pl.LazyFrame({"field1": ["test", "other"], "field2": ["test", "value"]})
...
def test_patch_multiple_fields():
    patches = {
        ...
        "type": {"^full$": "full planning permission"},
    }

    lf = pl.LazyFrame({"status": ["pending", "approved"], "type": ["full", "outline"]})
